...
{
    beginTest("Round trip");aurora::AuroraBroadcastProcessor a,b;auto* input=a.parameters.getParameter("inputTrimDb");input->setValueNotifyingHost(input->convertTo0to1(7.25f));juce::MemoryBlock state;a.getStateInformation(state);expect(state.getSize()>0);b.setStateInformation(state.getData(),static_cast<int>(state.getSize()));expect(b.lastRestoreWasValid());expectWithinAbsoluteError(b.parameters.getRawParameterValue("inputTrimDb")->load(),7.25f,0.001f);
