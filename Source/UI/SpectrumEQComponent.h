...
namespace aurora
{
class SpectrumEQComponent final:public juce::Component,public juce::SettableTooltipClient,private juce::Timer
{
public:
...
private:
    void timerCallback()override;juce::Rectangle<float> plotBounds()const;float frequencyToX(float)const;float xToFrequency(float)const;float gainToY(float)const;float yToGain(float)const;juce::Point<float> nodePosition(int)const;void setBandValue(const char*,float);
