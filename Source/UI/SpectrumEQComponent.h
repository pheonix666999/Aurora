...
private:
    void timerCallback()override;float frequencyToX(float)const;float xToFrequency(float)const;float gainToY(float)const;float yToGain(float)const;juce::Point<float> nodePosition(int)const;void setBandValue(const char*,float);
