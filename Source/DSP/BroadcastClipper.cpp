...
void BroadcastClipper::process(juce::AudioBuffer<float>& b,const ParameterSnapshot& p) noexcept
{
    ...
    for(size_t i=0;i<up.getNumSamples();++i){const auto d=drive.getNextValue(),m=mix.getNextValue(),style=modeMix.getNextValue();for(size_t ch=0;ch<up.getNumChannels();++ch){const auto x=up.getSample(ch,i);const auto driven=x*d;peakWarning=peakWarning||std::abs(driven)>1.0f;const auto soft=std::tanh(driven),firm=driven/std::sqrt(1.0f+driven*driven),wet=juce::jmap(style,soft,firm);up.setSample(ch,i,juce::jmap(m,x,wet));}}
