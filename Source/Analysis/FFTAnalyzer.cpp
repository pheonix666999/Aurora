...
namespace aurora
{
FFTAnalyzer::FFTAnalyzer(AnalyzerFIFO& f):fifo(f){smoothed.fill(-100);}
bool FFTAnalyzer::update(float smoothing)
{
    ...
    std::copy(history.begin(),history.end(),fftData.begin());std::fill(fftData.begin()+fftSize,fftData.end(),0);window.multiplyWithWindowingTable(fftData.data(),fftSize);fft.performFrequencyOnlyForwardTransform(fftData.data());const auto a=juce::jlimit(0.0f,0.98f,smoothing);
