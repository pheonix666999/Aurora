...
flowchart LR
  ...
  G --> H[Harmonic density] --> I[M/S width, balance, mono bass, correlation guard]
...
The enhancer uses bounded `tanh` density, tilt-derived tone, wet/dry mix, and post-shaper DC removal. Stereo processing uses mid/side width, asymmetric balance gains, a one-pole low-side removal below the mono-bass frequency, and a slow correlation guard that reduces widening rather than switching to mono.
